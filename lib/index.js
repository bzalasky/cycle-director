...
function createRouter$(router) {
  ...
  if (router.history) {
    ...
    // Find a better way
    return _rx2['default'].Observable.merge(_rx2['default'].Observable.fromEvent(document.body, 'click'));
  }

  return _rx2['default'].Observable.merge(_rx2['default'].Observable.fromEvent(document, 'load'), _rx2['default'].Observable.fromEvent(window, 'hashchange'));
}
